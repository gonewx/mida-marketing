...
- 成片：`out/MiDa-launch-film.mp4`（1920×1080 · 30fps · H.264 + AAC · 65 秒，内嵌封面）
- 封面图：`out/cover.jpg`（1920×1080，即视频第 0 帧，可用作平台上传封面／分享缩略图）
...
|---|---|---|---|
| 0–2s | 封面 | 品牌名、标语、三大功能、网址 + 三部手机（五維 / 命書 / 合盤） | 覓搭 MiDa · 有理由的連接 · Mobile Web 首發 · 手機瀏覽器打開 mida.gonewx.com |
| 2–7s | 开场 | 星空、金色轨道 | 茫茫人海，為什麼偏偏是 TA？ |
| 7–12s | 痛点 | 三张只有分数的匿名配对卡 | 一般的配對，只給你一個分數——卻說不出為什麼，也不知道第一句該說什麼。 |
| 12–17.5s | 品牌 | `logo-mark.png` | 覓搭 MiDa · 有理由的連接 · 以命理為內核，開啟有理由的遇見 |
| 17.5–24s | 01 生辰建檔 | 登錄與註冊 `97f5fcd2`、生辰建檔與獨立授權 `063c099c` | 生辰，是無法美化的真實。生辰資料獨立授權、註銷即刪，只用於為你排盤。 |
| 24–32s | 02 AI 命書 | 八字命書與專業排盤 `abe3b348`、西式占星星盤 `fc07fadf`、命書章節詳解 `d4ec7139` | 同一張盤，千人千面的深度敘事；每一段解讀，都能回溯到排盤依據。 |
| 32–39s | 03 每日五維 | 今日運勢五維評分 `26af4e3c` | 感情・事業・財富・健康・人際；評分與結論永久免費 |
| 39–48s | 04 雙人合盤 | 雙人合盤與契合度洞察 `8d94a905` | 關係亮點／互補之處／破冰話題卡；合盤報告，就是你們的第一句話。 |
| 48–54s | 05 邀友 | 合盤邀請與裂變分享 `726763ff`、社群宣傳卡 | 分享到 Instagram、LINE、WhatsApp，好友註冊後，雙方各得 1 張深讀券。 |
| 54–58s | 信任 | 四张信任卡 | 評分與結論永遠免費 · 生辰獨立授權 · 註銷即刪（帳號內自助註銷）· 18+ 成人專屬 |
| 58–65s | 结尾 | Logo + CTA + 二维码 | 有理由的連接 · AI 命盤 · 深度合盤 · 五維日運 · 手機瀏覽器打開 mida.gonewx.com · 無需下載 · 即開即用 · Mobile Web 首發 |

片头 2 秒是封面镜头，第 0 帧就完整可见，避免播放器和平台预览显示黑屏。`composition/index.html` 中的场景关键帧使用「正片时间」（视频时间减去 `COVER`），`scripts/music.py` 的 `COVER` 需与之保持一致。
...
npm run render     # 逐帧渲染 → out/MiDa-launch-film.mp4 + out/cover.jpg（需要 imageio-ffmpeg 或设置 FFMPEG）
...
- 要改文案、节奏或画面，编辑 `composition/index.html`：场景 DOM 加上底部的时间轴（`A()` / `rise()` / `scene()`）。用浏览器打开 `composition/index.html?t=27` 就能预览任意秒数（视频时间）。
